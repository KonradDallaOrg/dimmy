...
| **Codex subscription** LLM (OpenAI/ChatGPT via local `codex` CLI) | `codex.rs` | `dimmy_codex_status`/`_spawn_login`/`_ping`/`_recheck`/`_diagnostics`/`_binary_path` | Sibling of `claude_code.rs`; `codex exec -` with prompt on stdin. **A ChatGPT sub only serves its account-default model (`gpt-5.5`); other model ids → HTTP 400.** See [`docs/dev/codex-backend.md`](docs/dev/codex-backend.md) |
