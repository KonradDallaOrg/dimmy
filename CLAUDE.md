...
- **`staging-latest` must stay invisible to the prod prerelease channel — by withholding the Velopack manifest, NOT by the tag name.** Velopack's `GithubSource(prerelease:true)` reads release ASSETS (`releases.win.json` + `.nupkg`), not the git tag, so a non-semver tag does NOT hide it. `staging-auto-update.yml` therefore uploads ONLY `Setup.exe` + `Portable.zip` (manual sideload). It must NEVER upload `releases.win.json` / `Dimmy-*.nupkg`: that build is flavor=staging (license-staging + Stripe Test, different pubkey) yet ships under packId `Dimmy`, and `vpk` strips its version to a clean `0.6.59` which OUTRANKS `0.6.59-rc1` — so a paying prod user on the prerelease channel would auto-update INTO a staging binary, breaking their license (verifies against the staging pubkey → silent free/trial demotion) and pointing "Buy plan" at Stripe Test. **Invariant: anything discoverable under packId `Dimmy` MUST be a prod-flavor `release.yml` build.** Proper side-by-side auto-update is the separate `Dimmy-Staging` packId in `staging-tester.yml`. Burned 2026-06-16 (user downloaded 0.6.58 stable, enabled prerelease, got served the 0.6.59 STAGING build). The 2026-05-11 "add the manifest so prerelease sees staging" change was the root cause and has been reverted.
