...
public static class MeetingRecapHelpers
{
    ...
    /// <summary>Render the canonical recap markdown as HTML, for the rich
    /// half of the clipboard.
    ///
    /// Why: the Copy button put ONLY markdown on the clipboard, so pasting a
    /// recap into Outlook, Teams or Word produced raw `##` and `**` — which
    /// is what "send the recap to someone" actually means for most people,
    /// and it looked broken. Putting both formats on the clipboard fixes it
    /// with no new button and no new decision for the user: rich targets take
    /// the HTML, plain-text targets take the markdown, unchanged.
    ///
    /// Deliberately NOT a general Markdown engine. It handles exactly what
    /// <see cref="BuildMarkdownFromSections"/> emits — headings, bullets,
    /// numbered items, bold, italic, inline code — and treats anything else
    /// as text. A recap is generated by our own prompt, so the input shape is
    /// known; a full parser would be machinery we would then have to own.
    /// Everything is HTML-escaped before any markup is added.</summary>
    public static string MarkdownToHtml(string markdown)
    {
        ...
        void CloseList()
        {
            if (openList != null) { sb.Append("</").Append(openList).Append('>'); openList = null; }
        }
        ...
        foreach (var rawLine in markdown.Replace("\r\n", "\n").Split('\n'))
        {
            ...
            // The invisible type tag is bookkeeping, not content.
            if (trimmed.StartsWith("<!--", StringComparison.Ordinal)) continue;

            if (trimmed.Length == 0) { CloseList(); continue; }

            if (trimmed.StartsWith("### ", StringComparison.Ordinal))
            { CloseList(); sb.Append("<h3>").Append(Inline(trimmed[4..])).Append("</h3>"); continue; }
            if (trimmed.StartsWith("## ", StringComparison.Ordinal))
            { CloseList(); sb.Append("<h2>").Append(Inline(trimmed[3..])).Append("</h2>"); continue; }
            if (trimmed.StartsWith("# ", StringComparison.Ordinal))
            { CloseList(); sb.Append("<h1>").Append(Inline(trimmed[2..])).Append("</h1>"); continue; }

            if (trimmed.StartsWith("- ", StringComparison.Ordinal)
                || trimmed.StartsWith("* ", StringComparison.Ordinal))
            { OpenList("ul"); sb.Append("<li>").Append(Inline(trimmed[2..])).Append("</li>"); continue; }

            var numbered = System.Text.RegularExpressions.Regex.Match(trimmed, @"^(\d+)\.\s+(.*)$");
            if (numbered.Success)
            { OpenList("ol"); sb.Append("<li>").Append(Inline(numbered.Groups[2].Value)).Append("</li>"); continue; }
    ...
    /// <summary>Escape, then apply inline markup. Escaping FIRST is what
    /// keeps a transcript containing `&lt;script&gt;` from becoming one.</summary>
    private static string Inline(string text)
    {
        var s = text.Replace("&", "&amp;").Replace("<", "&lt;").Replace(">", "&gt;");
        // Bold before italic: `**x**` would otherwise be read as two italics.
        s = System.Text.RegularExpressions.Regex.Replace(s, @"\*\*(.+?)\*\*", "<strong>$1</strong>");
        s = System.Text.RegularExpressions.Regex.Replace(s, @"(?<!\*)\*(?!\s)(.+?)(?<!\s)\*(?!\*)", "<em>$1</em>");
