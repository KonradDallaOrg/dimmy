...
extension DimmyCore {
    ...
    /// The language actually SPOKEN in an audio file, as an English name
    /// ("Italian"), or nil when it could not be established. Advisory by
    /// design: the caller keeps whatever wording it already uses on nil, and
    /// nothing may fail because of it. Blocking, ~1.4 s — call off the main
    /// thread.
    func detectAudioLanguage(path: String) -> String? {
        let cap = 64
        let buffer = UnsafeMutablePointer<CChar>.allocate(capacity: cap)
        defer { buffer.deallocate() }
        buffer[0] = 0
        let written = path.withCString { p in
            dimmy_detect_audio_language(p, buffer, Int32(cap))
        }
        guard written > 0 else { return nil }
    ...
    /// Localized "this summary was generated by AI" notice shown above a
    /// recap (EU AI Act art. 50(5)). `kind` is "title" or "hint". The core
    /// owns the wording so macOS, Windows and Linux say the same thing in the
    /// same six languages. Pure — safe to call before init.
    func aiNoticeText(kind: String, lang: String) -> String? {
        let cap = 1024
        let buffer = UnsafeMutablePointer<CChar>.allocate(capacity: cap)
        defer { buffer.deallocate() }
        buffer[0] = 0
        let written = kind.withCString { k in
            lang.withCString { l in
                dimmy_ai_notice_text(k, l, buffer, Int32(cap))
            }
        }
        guard written > 0 else { return nil }
        return String(cString: buffer)
