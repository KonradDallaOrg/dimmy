...
class FluidAudioBridgeInternal {
    ...
    /// Qwen3AsrManager (macOS 15+). Held as AnyObject because a stored
    /// property cannot be marked potentially unavailable.
    private var qwenManager: AnyObject?

    init() {}

    // MARK: - Qwen3-ASR (Neural Engine, macOS 15+)

    func initializeQwen3(int8: Bool) throws {
        guard #available(macOS 15, iOS 18, *) else { throw BridgeError.unsupported }
        let semaphore = DispatchSemaphore(value: 0)
        var initError: Error?

        Task {
            do {
                let variant: Qwen3AsrVariant = int8 ? .int8 : .f32
                let dir = try await Qwen3AsrModels.download(variant: variant)
                let manager = Qwen3AsrManager()
                try await manager.loadModels(from: dir)
                self.qwenManager = manager
            } catch {
                initError = error
            }
            semaphore.signal()
    ...
    func transcribeQwen3(_ samples: [Float], language: String?) throws -> String {
        guard #available(macOS 15, iOS 18, *) else { throw BridgeError.unsupported }
        guard let manager = qwenManager as? Qwen3AsrManager else {
        ...
        Task {
            do {
                text = try await manager.transcribe(audioSamples: samples, language: language)
            } catch {
...
// MARK: - Qwen3-ASR FFI (Neural Engine, macOS 15+)

public typealias FluidProgressCallback = @convention(c) (Double, UnsafeMutableRawPointer?) -> Void

@_cdecl("fluidaudio_qwen3_supported")
public func fluidaudio_qwen3_supported() -> Int32 {
    if #available(macOS 15, iOS 18, *) { return 1 }
    return 0
}

@_cdecl("fluidaudio_qwen3_models_exist")
public func fluidaudio_qwen3_models_exist(_ int8: Int32) -> Int32 {
    guard #available(macOS 15, iOS 18, *) else { return 0 }
    let variant: Qwen3AsrVariant = int8 != 0 ? .int8 : .f32
    let dir = Qwen3AsrModels.defaultCacheDirectory(variant: variant)
    return Qwen3AsrModels.modelsExist(at: dir) ? 1 : 0
}

/// Blocking download of one variant into FluidAudio's cache. `cb` receives
/// the completed fraction (0...1) with `ctx`, from a background thread.
/// Returns 0 on success, -1 on failure, -2 below macOS 15.
@_cdecl("fluidaudio_qwen3_download")
public func fluidaudio_qwen3_download(
    _ int8: Int32,
    _ cb: FluidProgressCallback?,
    _ ctx: UnsafeMutableRawPointer?
) -> Int32 {
    guard #available(macOS 15, iOS 18, *) else { return -2 }
    let variant: Qwen3AsrVariant = int8 != 0 ? .int8 : .f32
    ...
    Task {
        do {
            try await Qwen3AsrModels.download(variant: variant, progressHandler: { progress in
                cb?(progress.fractionCompleted, UnsafeMutableRawPointer(bitPattern: ctxBits))
            })
        } catch {
            print("Qwen3 download error: \(error)")
            failed = true
        }
        semaphore.signal()
    }
...
/// Load one variant (downloading it first if it is missing). The first load
/// compiles the models for the Neural Engine. -2 below macOS 15.
@_cdecl("fluidaudio_qwen3_initialize")
public func fluidaudio_qwen3_initialize(_ ptr: UnsafeMutableRawPointer?, _ int8: Int32) -> Int32 {
    guard let ptr = ptr else { return -1 }
    let bridge = Unmanaged<FluidAudioBridgeInternal>.fromOpaque(ptr).takeUnretainedValue()
    do {
        try bridge.initializeQwen3(int8: int8 != 0)
        return 0
    } catch BridgeError.unsupported {
        return -2
    } catch {
        print("Qwen3 init error: \(error)")
        return -1
    }
}

/// Transcribe `count` 16 kHz mono Float32 samples. `language` NULL or empty
/// = automatic detection. The returned string must be freed with
/// `fluidaudio_free_string`.
@_cdecl("fluidaudio_qwen3_transcribe")
public func fluidaudio_qwen3_transcribe(
    ...
    guard let ptr = ptr, let samples = samples, count > 0 else { return -1 }
    let bridge = Unmanaged<FluidAudioBridgeInternal>.fromOpaque(ptr).takeUnretainedValue()
    let pcm = Array(UnsafeBufferPointer(start: samples, count: count))
    let lang = language.map { String(cString: $0) }.flatMap { $0.isEmpty ? nil : $0 }
    do {
        let text = try bridge.transcribeQwen3(pcm, language: lang)
        outText?.pointee = strdup(text)
        return 0
    } catch {
        print("Qwen3 transcribe error: \(error)")
        return -1
    }
}
