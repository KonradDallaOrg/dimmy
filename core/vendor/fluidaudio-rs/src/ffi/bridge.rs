...
extern "C" {
    ...
    // Qwen3-ASR (Neural Engine, macOS 15+)
    fn fluidaudio_qwen3_supported() -> i32;
    fn fluidaudio_qwen3_models_exist(int8: i32) -> i32;
    fn fluidaudio_qwen3_download(
        int8: i32,
        cb: Option<extern "C" fn(f64, *mut std::ffi::c_void)>,
        ctx: *mut std::ffi::c_void,
    ) -> i32;
    fn fluidaudio_qwen3_initialize(bridge: *mut std::ffi::c_void, int8: i32) -> i32;
    fn fluidaudio_qwen3_transcribe(
        bridge: *mut std::ffi::c_void,
        samples: *const f32,
        count: isize,
        language: *const i8,
...
/// Qwen3-ASR on the Neural Engine. Added in Dimmy's vendored copy.
impl FluidAudioBridge {
    pub fn qwen3_supported() -> bool {
        unsafe { fluidaudio_qwen3_supported() == 1 }
    }

    pub fn qwen3_models_exist(int8: bool) -> bool {
        unsafe { fluidaudio_qwen3_models_exist(int8 as i32) == 1 }
    }

    /// Blocking. `progress` gets the completed fraction from a Swift task
    /// thread while this thread waits, hence `Sync`.
    pub fn qwen3_download(int8: bool, progress: &(dyn Fn(f64) + Sync)) -> Result<(), String> {
        extern "C" fn trampoline(fraction: f64, ctx: *mut std::ffi::c_void) {
            // SAFETY: ctx is the `&&dyn Fn` below, alive for the whole
            // blocking call that the callback can only happen inside.
            let f = unsafe { &*(ctx as *const &(dyn Fn(f64) + Sync)) };
            f(fraction);
        }
        let ctx = &progress as *const &(dyn Fn(f64) + Sync) as *mut std::ffi::c_void;
        match unsafe { fluidaudio_qwen3_download(int8 as i32, Some(trampoline), ctx) } {
            0 => Ok(()),
            -2 => Err("Qwen3-ASR on the Neural Engine needs macOS 15".to_string()),
            code => Err(format!("Qwen3-ASR download failed ({code})")),
        }
    }

    pub fn initialize_qwen3(&self, int8: bool) -> Result<(), String> {
        match unsafe { fluidaudio_qwen3_initialize(self.ptr, int8 as i32) } {
            0 => Ok(()),
            -2 => Err("Qwen3-ASR on the Neural Engine needs macOS 15".to_string()),
            code => Err(format!("Qwen3-ASR init failed ({code})")),
        }
    }

    pub fn transcribe_qwen3(&self, samples: &[f32], language: Option<&str>) -> Result<String, String> {
        let lang = language
            .map(|l| CString::new(l).map_err(|e| e.to_string()))
            .transpose()?;
        let mut out: *mut i8 = std::ptr::null_mut();
        let rc = unsafe {
            fluidaudio_qwen3_transcribe(
                self.ptr,
                samples.as_ptr(),
        ...
        if rc != 0 || out.is_null() {
            return Err(format!("Qwen3-ASR transcribe failed ({rc})"));
        }
        let text = unsafe { CStr::from_ptr(out) }.to_string_lossy().into_owned();
        unsafe { fluidaudio_free_string(out) };
        Ok(text)
    }
}
