...
impl FluidAudio {
    ...
    /// True on macOS 15+, where FluidAudio's Qwen3-ASR pipeline can run.
    pub fn qwen3_supported() -> bool {
        ffi::FluidAudioBridge::qwen3_supported()
    }

    /// Whether the variant's CoreML models are already in FluidAudio's cache.
    pub fn qwen3_models_exist(int8: bool) -> bool {
        ffi::FluidAudioBridge::qwen3_models_exist(int8)
    }

    /// Download one variant (blocking), reporting the completed fraction.
    pub fn qwen3_download(
        int8: bool,
        progress: &(dyn Fn(f64) + Sync),
    ) -> Result<(), FluidAudioError> {
        ffi::FluidAudioBridge::qwen3_download(int8, progress).map_err(FluidAudioError::from)
    }

    /// Load one variant, downloading it first if needed. The first load
    /// compiles the models for the Neural Engine.
    pub fn init_qwen3(&self, int8: bool) -> Result<(), FluidAudioError> {
        self.bridge.initialize_qwen3(int8).map_err(FluidAudioError::from)
    }

    /// Transcribe 16 kHz mono f32 samples. `language` None = auto-detect.
    pub fn transcribe_qwen3(
        &self,
        samples: &[f32],
        language: Option<&str>,
    ) -> Result<String, FluidAudioError> {
        self.bridge
            .transcribe_qwen3(samples, language)
            .map_err(FluidAudioError::from)
    }
